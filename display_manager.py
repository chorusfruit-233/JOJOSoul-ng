...
class DisplayManager:
    ...
    def __init__(self, mode='both'):
        ...
        if mode in ['gui', 'both']:
            try:
                ...
                if mode == 'gui':
                    print("警告: easygui未安装，GUI功能将无法使用", file=sys.stderr)
    ...
    def use_terminal(self):
        """检查是否使用终端"""
        return self.mode == 'terminal' or self.mode == 'both' or (self.mode == 'gui' and not self.gui_available)
    ...
    def get_yes_no(self, title, message):
        ...
        # GUI模式 - 仅使用GUI
            ...
            result = self.easygui.buttonbox(message, title, choices=['是', '否'])
            return result == '是'
